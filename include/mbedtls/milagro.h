...
#define MBEDTLS_ERR_MILAGRO_CS_KEY_COMPUTATOIN_FAILED   -0x6500  /**< The client/server has failed computing the premaster secret. */
/*
 * MILAGRO_P2P errors
 */
#define MBEDTLS_ERR_MILAGRO_P2P_READ_PARAM_FAILED               -0x6480  /**< The client/server has failed reading the parameters. */
...
#define MBEDTLS_ERR_MILAGRO_BAD_PARAMETERS  -0x6320  /**< Invalid or missing initial parameters. */
...
struct mbedtls_milagro_cs_context
{
    ...
    octet G1;                 /*!< Parameter for the computation of the shared secret                     */
    octet G2;                 /*!< Parameter for the computation of the shared secret                     */
    int pin;                  /*!< 4 digit pin, is 0 if not required                                      */
#endif
#if defined(MBEDTLS_SSL_SRV_C)
    octet HID;                /*!< Parameter owned only by the server                                     */
#endif
    octet param_rand;         /*!< Random number internally generated by the server and client            */
    octet W;                  /*!< Public parameter sent to the client for the shared secret computation  */
    octet R;                  /*!< Public parameter sent to the server for the shared secret computation  */
    octet secret;             /*!< Secret provided by the TA (different for client and server)
                                    in case of required PIN, is the token                                 */
    octet U;                  /*!< It is the output x.H(ID),                                              */
    ...
    octet shared_secret;      /*!< Shared_secret generated at the end of the MILAGRO_CS protocol          */
    csprng RNG;               /*!< Random Number Generator                                                */
    int date;                 /*!< value set to 0                                                         */
    unsign32 timevalue;       /*!< current epoch time                                                     */
};
...
/**
 * \brief                   Check if the parameters are ok
 *
 * \param milagro_cs        milagro_cs struct which contains the parameters
 *                          to be initialized
 *
 ...
 *                  compute the shared secret (TLS: contents of the Client/ServerKeyExchange)
 *
...
/**
 * \brief           Read the public parameter for the computation of the shared secret
